...
class RLAB:
    def __init__(self, classifier, agent_config, device, ds_info):
        ...
        self.noise_severity_levels = agent_config['noise_info']['noise_severity_levels']
    ...
    def generate(self, x, y, idx=None):
        ...
        for step in trange(self.max_steps, desc="RLAB", disable=False if self.verbose in [1] else True):
            ...
            if is_done and self.l2_at_flip is None:
                self.l2_at_flip = l2
                if self.l2_at_flip == l2:  # adv sample
            ...
            if self.l2_at_flip is not None and is_done:  # adv examples with different noise severity levels
                reached_max_thr = False
                for i, thr in enumerate(sorted(self.noise_severity_levels, reverse=True)):
                    if l2 > self.l2_at_flip * thr:
                        file_name = os.path.join(cur_episode_dir, "c{}_x_{}_adv.jpg".format(gt_idx_cpu, thr))
                        self.save_img(to_original_format(
                            un_standardize(perturbed_image, self.dataset_mean, self.dataset_std, clip01=True)),
                            file_name)
                        if i == 0:
                            reached_max_thr = True
                            break
                if reached_max_thr:
                    break
